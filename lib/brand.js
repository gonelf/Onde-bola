 ...
 * the same product — their own wordmark, a pinned English locale and a pinned
 * home market, so the SEO copy, page titles, share images and the lead TV
 * country stay fixed regardless of where the visitor is (footietoday → UK,
 * footytoday → US). The default brand still follows the visitor's country.
 *
...
export const DEFAULT_BRAND = {
  ...
  lang: null,
  // null → default the primary TV country to the visitor's IP geo.
  country: null,
};
...
// Default primary TV country for a brand: an English-only domain pins its home
// market (footietoday → UK, footytoday → US); otherwise fall back to whatever
// country the caller resolved (typically the visitor's IP geo).
export function countryForBrand(brand, fallbackCountry) {
  return (brand && brand.country) || fallbackCountry;
}
