...
// Returns { ok, fields, assetTypeName, assetFields } — createPost's input fields
// plus the resolved `assets` element type's fields, so we know exactly how to
// attach the image.
export async function introspectCreatePost() {
  const token = process.env.BUFFER_ACCESS_TOKEN || "";
  if (!token) return { ok: false, fields: [], error: "BUFFER_ACCESS_TOKEN not set" };
  const main = await introspectType(token, "CreatePostInput");
  if (!main.ok) return main;

  let assetTypeName = null, assetFields = null;
  const assetsField = main.fieldsRaw.find((f) => f.name === "assets");
  if (assetsField) {
    assetTypeName = namedType(assetsField.type);
    if (assetTypeName) {
      const at = await introspectType(token, assetTypeName);
      if (at.ok) assetFields = at.fields;
    }
  }
  return { ok: true, fields: main.fields, assetTypeName, assetFields };
}
