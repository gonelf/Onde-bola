...
const COPY = {
  en: {
    ...
    title: (vs, comp, brand) => `Where to watch ${vs}${comp ? " — " + comp : ""} on TV · ${brand}`,
    ...
    leagueTitle: (name, brand) => `${name} on TV — where to watch every match · ${brand}`,
    ...
    classification: "Classification", clsClub: "Club",
    clsHeads: ["P", "W", "D", "L", "GF", "GA", "GD", "Pts"],
  },
  pt: {
    crumbHome: "Início", vs: "vs", ftShort: "FIM",
    defaultHome: "Casa", defaultAway: "Fora",
    heading: (vs) => `Onde ver ${vs}`,
    title: (vs, comp, brand) => `Onde ver ${vs}${comp ? " — " + comp : ""} na TV · ${brand}`,
    ...
    leagueTitle: (name, brand) => `${name} na TV — onde ver todos os jogos · ${brand}`,
    ...
    classification: "Classificação", clsClub: "Equipa",
    clsHeads: ["J", "V", "E", "D", "GM", "GS", "DG", "P"],
  },
};
...
export async function buildShare({ origin, lang, parts, query, auth }) {
  ...
  const title = c.title(vs, compDisplay, brandName);
  ...
  const crumbs = [{ name: brandName, item: origin + "/" }];
  ...
  const bodyHtml = `<div class="wrap">
    ...
    <footer>
      <a href="/">${esc(brandName)}</a> — ${esc(c.footerTail)}
  ...
  return {
    ok: true,
    robots, title, description, headline, siteName: brandName,
...
const LEAGUE_CSS = `
  ...
  *{box-sizing:border-box}
  ...
  .classification{margin:18px 0}
  .classification>h2{font-size:.85rem;text-transform:uppercase;letter-spacing:.04em;color:var(--muted);margin:0 0 8px}
  .classification h3.grp{font-size:.78rem;text-transform:uppercase;letter-spacing:.04em;color:var(--muted);margin:12px 0 6px}
  .tbl-scroll{overflow-x:auto;background:var(--panel);border:1px solid var(--line);border-radius:10px}
  table.table{width:100%;border-collapse:collapse;font-size:.82rem;white-space:nowrap}
  table.table th{color:var(--muted);font-weight:600;font-size:.7rem;text-align:right;padding:7px 6px;border-bottom:1px solid var(--line)}
  table.table td{text-align:right;padding:8px 6px;border-bottom:1px solid var(--line);color:var(--muted)}
  table.table tbody tr:last-child td{border-bottom:0}
  table.table .p{text-align:center;width:1.8rem;color:var(--txt)}
  table.table .c{text-align:left;width:99%;color:var(--txt);font-weight:600}
  table.table .pts{color:var(--txt);font-weight:700}
...
export async function buildLeague({ origin, lang, leagueSlug, auth }) {
  ...
  const title = c.leagueTitle(leagueName, brandName);
  ...
  // Classification: fetch the competition's table (same FotMob-backed endpoint
  // the live app uses) and render it above the fixtures. Best-effort — a cup /
  // international with no table, or an upstream blip, just omits the section.
  const leagueId = (games.find((f) => f.leagueId != null) || {}).leagueId;
  let standings = null;
  if (leagueId != null) {
    const st = await getJson(`${origin}/api/standings?league=${leagueId}`, 6000, { headers: auth, retries: 1 });
    if (st && st.ok && Array.isArray(st.groups) && st.groups.length) standings = st;
  }
  const tableSection = standings ? `<section class="classification">
    <h2>${esc(c.classification)}</h2>
    ${standings.groups.map((g) => `${standings.groups.length > 1 && g.name ? `<h3 class="grp">${esc(g.name)}</h3>` : ""}
      <div class="tbl-scroll"><table class="table">
        <thead><tr><th class="p">#</th><th class="c">${esc(c.clsClub)}</th>${c.clsHeads.map((hd) => `<th>${esc(hd)}</th>`).join("")}</tr></thead>
        <tbody>${g.rows.map((r, i) => {
          const qhex = /^#[0-9a-f]{3,8}$/i.test(String(r.qual || "")) ? r.qual : "";
          const gd = r.gd > 0 ? "+" + r.gd : String(r.gd);
          return `<tr>
            <td class="p"${qhex ? ` style="box-shadow:inset 3px 0 0 ${qhex}"` : ""}>${esc(r.rank || i + 1)}</td>
            <td class="c">${esc(r.name)}</td>
            <td>${esc(r.played)}</td><td>${esc(r.won)}</td><td>${esc(r.drawn)}</td><td>${esc(r.lost)}</td>
            <td>${esc(r.gf)}</td><td>${esc(r.ga)}</td><td>${esc(gd)}</td><td class="pts">${esc(r.points)}</td>
          </tr>`;
        }).join("")}</tbody>
      </table></div>`).join("")}
  </section>` : "";
  ...
  const breadcrumb = {
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: brandName, item: origin + "/" },
  ...
  return {
    ok: true,
    robots, title, description, siteName: brandName,
