...
import { buildTextPost, buildMatchCaption } from "@/lib/digest-text";
...
// Twitter/X and the others don't, so omit it. `postType` picks the enum value —
// `post` for the image cards, `reel` for the daily video.
function metadataFor(service, postType) {
  const s = String(service || "").toLowerCase();
  const type = postType === "reel" ? "reel" : "post";
  if (s.includes("instagram")) {
    return `{ instagram: { type: ${type}, shouldShareToFeed: true } }`;
  }
  if (s.includes("facebook")) {
    return `{ facebook: { type: ${type} } }`;
  }
  return "";
}
...
// card is attached as an image asset — or, for the daily reel, the mp4 as a video
// asset (with its cover as the thumbnail); `service` drives any required metadata.
async function createPostOnce({ token, channelId, text, imageUrl, videoUrl, thumbnailUrl, dueAt, service, postType }) {
  // `assets` is a REQUIRED field ([AssetInput!]!). Attach the card as an image
  // asset (or the reel as a video asset); text-only posts pass an empty list.
  const assets = videoUrl
    ? `[{ video: { url: ${gqlStr(videoUrl)}${thumbnailUrl ? `, thumbnailUrl: ${gqlStr(thumbnailUrl)}` : ""} } }]`
    : imageUrl ? `[{ image: { url: ${gqlStr(imageUrl)} } }]` : `[]`;
  const meta = metadataFor(service, postType);
...
export async function scheduleBufferUpdate({ text, photo, scheduledAt }) {
  ...
  } catch (e) { /* no services → full text, no metadata */ }
...
// Schedule the reel on one channel: try as a Reel (metadata type: reel), and if
// Buffer rejects that — e.g. a channel/plan where the reel enum isn't accepted —
// retry once as a regular video post so the video still goes out. Mirrors the
// image post's drop-the-image retry. Never falls back to text-only: a reel
// without its video is pointless.
async function createReelPost({ token, channelId, text, videoUrl, thumbnailUrl, dueAt, service }) {
  const base = { token, channelId, text, videoUrl, thumbnailUrl, dueAt, service };
  const first = await createPostOnce({ ...base, postType: "reel" });
  if (first.ok) return { channelId, asReel: true, ...first };

  const retry = await createPostOnce({ ...base, postType: "post" });
  if (retry.ok) {
    return {
      channelId,
      ...retry,
      asReel: false,
      message: `scheduled as video POST, not reel — reel rejected (${first.message})`,
    };
  }
  return {
    channelId,
    ...
    status: first.status,
    postId: null,
    message: `as reel: ${first.message} || as post: ${retry.message}`,
  };
}

// Schedule a video post on the account's Facebook + Instagram channels only
// (the platforms with a Reels surface — other configured channels are left to
// the image post). Returns { ok, status, results, channelSource } or
// { fail: <message> } when nothing could even be attempted. Shared by the
// daily reel and the match-replay posts.
async function scheduleVideoOnFbIg({ token, text, videoUrl, thumbUrl, scheduledAt }) {
  const { ids, source } = await resolveChannelIds();
  if (!ids.length) return { fail: "No Buffer channels configured — set them in /admin/buffer or BUFFER_CHANNEL_IDS" };
  const svcById = {};
  try {
    const disc = await listBufferChannels();
    if (disc && disc.ok) disc.channels.forEach((c) => { if (c && c.id) svcById[c.id] = c.service; });
  } catch (e) { /* no discovery → no targets, reported below */ }
  const targets = ids.filter((id) => {
    const s = String(svcById[id] || "").toLowerCase();
    return s.includes("facebook") || s.includes("instagram");
  });
  if (!targets.length) return { fail: "no Facebook/Instagram channels among the configured Buffer channels" };
  ...
  for (const channelId of targets) {
    ...
    const channelText = fitCaption(text, captionLimitFor(service));
    try {
      results.push(await createReelPost({ token, channelId, text: channelText, videoUrl, thumbnailUrl: thumbUrl || "", dueAt: scheduledAt, service }));
    } catch (e) {
      results.push({ channelId, ok: false, status: 0, postId: null, message: String((e && e.message) || e) });
    }
  }
...
// Build one video post's log entry, schedule it on FB+IG, log and return it.
async function scheduleVideoPost({ trigger, date, scheduledAt, videoUrl, text, thumbUrl }) {
  const at = new Date().toISOString();
  const token = process.env.BUFFER_ACCESS_TOKEN || "";

  const fail = async (message) => {
    const entry = { at, trigger, date, scheduledAt, videoUrl, ok: false, status: 0, message };
    await appendBufferLog(entry);
    return entry;
  };

  if (!token) return fail("Buffer not configured (BUFFER_ACCESS_TOKEN)");
  if (!/^https:\/\//.test(videoUrl || "")) return fail("no public video URL");
  if (!text) return fail("could not build post text");

  const res = await scheduleVideoOnFbIg({ token, text, videoUrl, thumbUrl, scheduledAt });
...
/**
 * Build and schedule the day's video reel on the account's Facebook + Instagram
 * channels, then append the outcome to the log. Called by /api/cron-buffer-reel
 * after the GitHub Actions renderer uploads the mp4.
 *   origin    request origin, for the brand/caption
 *   date      YYYY-MM-DD the reel is about (publishes 09:00 UTC that day)
 *   videoUrl  public mp4 URL (Vercel Blob) Buffer fetches itself
 *   thumbUrl  optional public cover-image URL
 *   trigger   "cron" | "manual" — recorded in the log as reel-<trigger>
 * Returns the log entry (with `ok`).
 */
export async function scheduleReelPost({ origin, date, videoUrl, thumbUrl, trigger, auth }) {
  // Same caption as the image post, built in-process (see scheduleDayPost for
  // why it must never be fetched over HTTP).
  const lang = brandForOrigin(origin).lang || "pt";
  let text = "";
  try {
    const built = await buildTextPost({ origin, date, n: "", lang, auth });
    text = (built && typeof built.text === "string" ? built.text : "").trim();
  } catch (e) { /* scheduleVideoPost reports the empty caption */ }

  return scheduleVideoPost({
    trigger: trigger === "cron" ? "reel-cron" : "reel-manual",
    date,
...
/**
 * Schedule the animated match-replay video of one finished game on Facebook +
 * Instagram: the day's top game, recorded off /admin/replay by
 * .github/workflows/replay-cron.yml. Publishes the morning after the match at
 * 10:00 UTC — an hour after the games-of-the-day posts, so they don't collide.
 *   origin    request origin, for the brand/caption
 *   date      YYYY-MM-DD the match was PLAYED (not the publish day)
 *   fmid      the game's id in the fixtures feed, to build the caption
 *   videoUrl  public mp4 URL (Vercel Blob) Buffer fetches itself
 *   thumbUrl  optional public cover-image URL
 *   trigger   "cron" | "manual" — recorded in the log as replay-<trigger>
 * Returns the log entry (with `ok`).
 */
export async function scheduleReplayPost({ origin, date, fmid, videoUrl, thumbUrl, trigger, auth }) {
  const brand = brandForOrigin(origin);
  const lang = brand.lang || "pt";

  // The caption needs the final score — read the game back from the fixtures
  // feed for the match day (in-process fetch, same pattern as buildTextPost).
  let text = "";
  try {
    const r = await fetch(`${origin}/api/fixtures?date=${date}&all=1`, {
      headers: Object.assign({ Accept: "application/json" }, auth || {}),
    });
    const j = r.ok ? await r.json() : null;
    const game = ((j && j.fixtures) || []).find((f) => String(f.fmid) === String(fmid));
    if (game) text = buildMatchCaption({ game, lang, site: brand.domain }).trim();
  } catch (e) { /* scheduleVideoPost reports the empty caption */ }

  const publish = new Date(`${date}T00:00:00.000Z`);
  publish.setUTCDate(publish.getUTCDate() + 1);
  const scheduledAt = `${publish.toISOString().slice(0, 10)}T10:00:00.000Z`;

  return scheduleVideoPost({
    trigger: trigger === "cron" ? "replay-cron" : "replay-manual",
    date,
    scheduledAt,
    videoUrl,
    thumbUrl,
