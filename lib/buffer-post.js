 ...
 * channels (the platforms with a Reels surface — other configured channels are
 * left to the image post), then append the outcome to the log. Called by
 * /api/cron-buffer-reel after the GitHub Actions renderer uploads the mp4.
...
export async function scheduleReelPost({ origin, date, videoUrl, thumbUrl, trigger, auth }) {
  const scheduledAt = `${date}T09:00:00.000Z`;
  const at = new Date().toISOString();
  const src = trigger === "cron" ? "reel-cron" : "reel-manual";
  const token = process.env.BUFFER_ACCESS_TOKEN || "";

  const fail = async (message) => {
    const entry = { at, trigger: src, date, scheduledAt, videoUrl, ok: false, status: 0, message };
    await appendBufferLog(entry);
    return entry;
  };
  ...
  // Reels go to Facebook + Instagram only; resolve the configured channels and
  // keep the ones on those services.
  const { ids, source } = await resolveChannelIds();
  ...
  try {
    const disc = await listBufferChannels();
    if (disc && disc.ok) disc.channels.forEach((c) => { if (c && c.id) svcById[c.id] = c.service; });
  } catch (e) { /* no discovery → no reel targets, reported below */ }
  ...
  const entry = {
    ...
    results,
    message: summarizeResults({ results, ok }),
    textPreview: text.slice(0, 280),
  };
  await appendBufferLog(entry);
  return entry;
