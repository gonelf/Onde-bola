...
const CANVAS_FORMATS = {
  landscape: { W: 1200, H: 630, pad: 56, accent: 10, brandDot: 24, brandFont: 28, titleFont: 40, dateFont: 28, tagFont: 20, rowH: 92, rowGap: 10, eyebrowBand: 28, vsFont: 20, eyebrowFont: 22, eyebrowLabelFont: 15, railCol: 150, railTime: 34, railLabel: 16, railGap: 5, nameFont: 28, crest: 46, scoreFont: 30, badge: 34, scoreHalf: 78, gap: 16, heroH: 270, heroGap: 16, heroCompFont: 22, heroName: 36, heroCrest: 96, heroScore: 64, heroVsFont: 34, heroEyebrowFont: 32, heroEyebrowLabelFont: 21, heroBadge: 32, heroScoreHalf: 96, emptyFont: 40 },
  square: { W: 1080, H: 1080, pad: 64, accent: 12, brandDot: 28, brandFont: 32, titleFont: 48, dateFont: 28, tagFont: 22, rowH: 122, rowGap: 14, eyebrowBand: 34, vsFont: 23, eyebrowFont: 26, eyebrowLabelFont: 18, railCol: 168, railTime: 40, railLabel: 18, railGap: 6, nameFont: 32, crest: 56, scoreFont: 34, badge: 40, scoreHalf: 84, gap: 18, heroH: 370, heroGap: 20, heroCompFont: 26, heroName: 44, heroCrest: 140, heroScore: 84, heroVsFont: 40, heroEyebrowFont: 40, heroEyebrowLabelFont: 26, heroBadge: 42, heroScoreHalf: 120, emptyFont: 46 },
  story: { W: 1080, H: 1920, pad: 72, accent: 16, brandDot: 34, brandFont: 40, titleFont: 60, dateFont: 34, tagFont: 28, rowH: 150, rowGap: 18, eyebrowBand: 42, vsFont: 26, eyebrowFont: 31, eyebrowLabelFont: 21, railCol: 210, railTime: 50, railLabel: 22, railGap: 7, nameFont: 40, crest: 68, scoreFont: 44, badge: 50, scoreHalf: 96, gap: 20, heroH: 480, heroGap: 26, heroCompFont: 32, heroName: 56, heroCrest: 184, heroScore: 112, heroVsFont: 52, heroEyebrowFont: 48, heroEyebrowLabelFont: 30, heroBadge: 54, heroScoreHalf: 150, emptyFont: 58 },
};
...
export function buildImage({ date, n, format, highlight, lang }) {
  ...
  (function () {
    ...
    // VIVO"; finished → dimmed HH:MM + "FIM". Anchored at left x, centred on cy.
    function drawRail(f, x, cy, S) {
      var state = matchState(f), time = fmtTime(f.kickoff);
      ctx.textBaseline = "middle"; ctx.textAlign = "left";
      if (state === "live") {
        var minute = (f.status || L.live); if (minute.length > 6) minute = minute.slice(0, 6);
        var ty = cy - (S.railLabel + S.railGap) / 2, ly = cy + (S.railTime + S.railGap) / 2;
        setFont(S.railTime, "800"); ctx.fillStyle = C.live; ctx.fillText(minute, x, ty);
        var dot = S.railLabel * 0.34;
        circle(x + dot, ly, dot, C.live);
        setFont(S.railLabel, "800"); ctx.fillStyle = C.live; ctx.fillText(L.live, x + dot * 2 + S.railLabel * 0.4, ly);
        return;
      }
      if (state === "finished") {
        var ty2 = cy - (S.railLabel + S.railGap) / 2, ly2 = cy + (S.railTime + S.railGap) / 2;
        setFont(S.railTime, "800"); ctx.fillStyle = C.eyebrowTimeFin; ctx.fillText(time || "—", x, ty2);
        setFont(S.railLabel, "800"); ctx.fillStyle = C.muted; ctx.fillText(L.ft, x, ly2);
        return;
      }
      // scheduled
      setFont(S.railTime, "800"); ctx.fillStyle = C.eyebrowTime; ctx.fillText(time || "", x, cy);
    }
