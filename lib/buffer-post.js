...
// env/auto-default).
//
// Writes Postgres (transactional replace-all) AND mirrors to the legacy KV key
// that getStoredChannelIds reads as a fallback. Mirroring keeps Save working even
// when Postgres is unavailable or the buffer_channels table hasn't been migrated
// yet — the selection still persists and reads back. Only throws (so /api/buffer
// surfaces a real failure instead of a false "saved ✓") when BOTH stores fail.
export async function saveStoredChannelIds(ids) {
  ...
  let kvOk = false;
  try {
    kvOk = (await kv(["SET", LEGACY_CHANNELS_KEY, JSON.stringify(clean)])) !== null;
  } catch (e) {
    kvOk = false;
  }

  if (!dbOk && !kvOk) {
    throw new Error("could not persist channel selection — Postgres and KV both unavailable");
  }
  return clean;
}
