...
// Pull this match's broadcast rows out of the day's FotMob TV feed, grouped by
// country (Portugal first).
...
export async function buildShare({ origin, parts, query }) {
  parts = parts || [];
  query = query || {};
...
export async function buildLeague({ origin, leagueSlug }) {
  ...
  return {
    ...
    headline: `${leagueName} on TV — where to watch`,
