...
export function buildImage({ date, n, format, highlight, lang }) {
  ...
  (function () {
    ...
    function drawChannelPill(x, cy, ch, paid, S) {
      ...
      var fs = S.railChannel, padH = Math.round(fs * 0.5), padV = Math.round(fs * 0.24);
      var lockW = paid ? fs * 0.78 : 0, lockGap = paid ? fs * 0.26 : 0;
      setFont(fs, "700");
      var label = ellipsize(ch, S.railCol - padH * 2 - lockW - lockGap - 2);
    ...
    function drawRail(f, x, cy, S) {
      ...
      if (ch) {
        ...
        lines.push({ h: S.railChannel + Math.round(S.railChannel * 0.24) * 2, draw: function (yc) {
          drawChannelPill(x, yc, ch, paid, S);
        } });
      }
