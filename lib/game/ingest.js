...
import { canonicalOf } from "@/lib/game/clubAliases";
...
// Does b enrich a? The alias table is authoritative: when BOTH names resolve to
// a canonical id, equality decides — fixing short/long misses AND blocking
// false merges of distinct clubs that share a token (Sheffield United/Wednesday,
// Sporting CP/Braga). Otherwise fall back to fuzzy key/containment/token match.
function sameClub(a, b) {
  const ca = canonicalOf(a), cb = canonicalOf(b);
