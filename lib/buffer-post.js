...
// Buffer throttles aggressively (HTTP 429 "Too many requests from this client");
// a brief honoured-Retry-After backoff with a couple of retries keeps a transient
// throttle from surfacing as "no channels found". Capped so it can't hang the
// serverless function.
async function bufferGraphQL(query, token, attempt = 0) {
  ...
  if ((r.status === 429 || r.status === 503) && attempt < 2) {
    ...
    const waitMs = Math.min(Number.isFinite(ra) && ra > 0 ? ra * 1000 : (attempt + 1) * 1000, 3000);
    await sleep(waitMs);
