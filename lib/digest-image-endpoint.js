 ...
 *
 * The same handler also backs the dated path /image/<date>/square (and
 * /image/today/square), reading the day from the route segment.
 */
...
// When mounted under a dynamic `[date]` segment (e.g. /image/<date>/square) the
// matched date arrives via ctx.params.date and takes precedence over ?date=.
export function digestImageHandler(publicFormat) {
  ...
  return async function GET(req, ctx) {
    ...
    // A YYYY-MM-DD path segment ("today" maps to its own static route) wins over
    // ?date=; anything else falls back to the query (renderer defaults to today).
    const pathDate = /^\d{4}-\d{2}-\d{2}$/.test(params.date || "") ? params.date : "";
    const date = pathDate || src.searchParams.get("date");
