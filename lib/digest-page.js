...
// Competition prominence by FotMob league id (kept in sync with /api/og's
// digest), most-prominent first. Ranking by id — not name — avoids false
// matches like Tanzania's "Premier League" outranking the real one.
...
async function renderToday(res, { origin, date, n }) {
  ...
  // all=1: rank every competition for the date here, independent of the fixtures
  // "major leagues" allow-list (which can miss off-season tournaments).
