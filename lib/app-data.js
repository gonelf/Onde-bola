...
export function groupByCountry(tv) {
  ...
  (tv || []).forEach(function (t) {
    ...
    // Fold streaming-brand spelling variants (e.g. "Amazon Prime Video" ->
    // "Prime Video") to a single canonical pill, so the same service doesn't
    // render — or dedup — as several different channels on one game.
    var channel = canonicalChannelName(t.channel);
    if (!byCountry[c]) byCountry[c] = [];
    if (byCountry[c].indexOf(channel) === -1) byCountry[c].push(channel);
  });
  return byCountry;
}
...
export function fetchRichListings(day) {
  return fetch("/api/listings?date=" + day, { headers: { Accept: "application/json" } })
    ...
    .then(function (d) { return { matches: (d && d.matches) || {}, refreshing: !!(d && d.refreshing) }; })
    .catch(function () { return { matches: {}, refreshing: false }; });
}
...
// The classification (league table) + next round of fixtures for a competition,
// keyed by its FotMob league id. Session-cached so reopening a competition is
// instant and a silent refresh tick doesn't re-fetch. Returns
// { ok, name, groups:[{name,rows}], next:{round,matches:[…]} }; degrades to an
// empty shape for cups / internationals with no table or no upcoming matches.
var standingsCache = {};
export function fetchStandings(leagueId) {
  var id = String(leagueId == null ? "" : leagueId);
  var empty = { ok: false, groups: [], next: { round: null, matches: [] } };
  if (!id) return Promise.resolve(empty);
  if (standingsCache[id]) return Promise.resolve(standingsCache[id]);
  return fetch("/api/standings?league=" + encodeURIComponent(id), { headers: { Accept: "application/json" } })
    .then(function (r) { if (!r.ok) throw new Error("standings " + r.status); return r.json(); })
    .then(function (d) {
      var out = {
        ok: !!(d && d.ok),
        name: (d && d.name) || "",
        groups: (d && d.groups) || [],
        next: (d && d.next) || { round: null, matches: [] },
      };
      standingsCache[id] = out;
      return out;
    })
