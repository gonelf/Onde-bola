...
// without reading KV on every request. Admin saves call revalidateTag(FLAGS_TAG)
// to bust this immediately; the short `revalidate` is only a backstop for any
// instance the tag invalidation doesn't reach, so a saved change is live within
// ~30s worst case (usually instantly via the tag).
