 ...
 *
 * Stored in one KV key, like lib/overrides. When nothing is saved (or an empty
 * list is saved) the built-in defaults below are used, so ads keep working out
 * of the box and an empty save can't silently wipe everything.
 *
...
export function parseSnippet(snippet) {
  ...
  // A snippet that's pure JS (no tags at all) is an inline script, not markup.
  if (!scripts.length && html && html.indexOf("<") === -1) {
    scripts.push({ code: html });
    html = "";
  }
  return { html, scripts };
}
...
// built-in defaults when nothing meaningful is stored (unset key or empty list).
export async function loadAds() {
  ...
  const list = (stored && stored.length)
    ? stored
