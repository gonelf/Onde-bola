...
// --- Overrides backed by Postgres (feature_flags), falling back to KV when the
// DB is unset/unreachable. First DB read backfills the existing KV overrides
...
async function readOverridesDB() {
  const rows = await db.select({ id: featureFlags.id, state: featureFlags.state }).from(featureFlags);
  const map = {};
  for (const r of rows) map[r.id] = r.state;
  return map;
}

async function writeOverridesDB(map) {
  const m = (map && typeof map === "object") ? map : {};
  const values = Object.keys(m).map((id) => ({ id, state: String(m[id]) }));
  await db.transaction(async (tx) => {
    await tx.delete(featureFlags);
    if (values.length) await tx.insert(featureFlags).values(values);
  });
}
...
export async function saveFlags(updates) {
  ...
  if (!db) {
    ...
    try {
      await writeOverridesDB(overrides);
      await markMigrated(FLAGS_KEY);
    } catch (e) {
      // Once migrated, reads no longer consult KV, so a KV write here would be a
      // silent no-op. Surface the failure instead; only fall back to KV while
      // the store is still pre-migration (reads still use it).
      if (await isMigrated(FLAGS_KEY)) throw e;
      await kv(["SET", FLAGS_KEY, JSON.stringify(overrides)]);
    }
  }
  return loadFlags();
