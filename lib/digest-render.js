...
const CANVAS_FORMATS = {
  landscape: { W: 1200, H: 630, pad: 56, accent: 10, brandDot: 24, brandFont: 28, titleFont: 40, dateFont: 28, tagFont: 20, rowH: 92, rowGap: 10, rowTimeCol: 130, eyebrowBand: 28, vsFont: 20, eyebrowFont: 26, eyebrowLabelFont: 16, nameFont: 28, crest: 46, scoreFont: 30, badge: 34, scoreHalf: 78, gap: 16, heroH: 270, heroGap: 16, heroCompFont: 22, heroName: 36, heroCrest: 96, heroScore: 64, heroVsFont: 34, heroEyebrowFont: 32, heroEyebrowLabelFont: 21, heroBadge: 32, heroScoreHalf: 96, emptyFont: 40 },
  square: { W: 1080, H: 1080, pad: 64, accent: 12, brandDot: 28, brandFont: 32, titleFont: 48, dateFont: 28, tagFont: 22, rowH: 122, rowGap: 14, rowTimeCol: 152, eyebrowBand: 34, vsFont: 23, eyebrowFont: 30, eyebrowLabelFont: 18, nameFont: 32, crest: 56, scoreFont: 34, badge: 40, scoreHalf: 84, gap: 18, heroH: 370, heroGap: 20, heroCompFont: 26, heroName: 44, heroCrest: 140, heroScore: 84, heroVsFont: 40, heroEyebrowFont: 40, heroEyebrowLabelFont: 26, heroBadge: 42, heroScoreHalf: 120, emptyFont: 46 },
  story: { W: 1080, H: 1920, pad: 72, accent: 16, brandDot: 34, brandFont: 40, titleFont: 60, dateFont: 34, tagFont: 28, rowH: 150, rowGap: 18, rowTimeCol: 196, eyebrowBand: 42, vsFont: 26, eyebrowFont: 38, eyebrowLabelFont: 21, nameFont: 40, crest: 68, scoreFont: 44, badge: 50, scoreHalf: 96, gap: 20, heroH: 480, heroGap: 26, heroCompFont: 32, heroName: 56, heroCrest: 184, heroScore: 112, heroVsFont: 52, heroEyebrowFont: 48, heroEyebrowLabelFont: 30, heroBadge: 54, heroScoreHalf: 150, emptyFont: 58 },
};
...
export function buildImage({ date, n, format, highlight, lang }) {
  ...
  (function () {
    ...
    // Left rail (TV-guide style): kickoff time / live minute / finished time with
    // its label beneath, left-aligned at x. Centred vertically on cy.
    function drawTimeRail(f, x, cy, S) {
      var state = matchState(f), time = fmtTime(f.kickoff);
      ctx.textAlign = "left"; ctx.textBaseline = "middle";
      var prim = S.eyebrowFont, sub = S.eyebrowLabelFont, dy = prim * 0.5 + 4;
      if (state === "live") {
        var minute = (f.status || L.live); if (minute.length > 6) minute = minute.slice(0, 6);
        setFont(prim, "800"); ctx.fillStyle = C.live; ctx.fillText(minute, x, cy - dy);
        setFont(sub, "800"); ctx.fillStyle = C.live; ctx.fillText("• " + L.live, x, cy + sub * 0.7 + 2);
        return;
      }
      if (state === "finished") {
        setFont(prim, "800"); ctx.fillStyle = C.eyebrowTimeFin; ctx.fillText(time || "—", x, cy - dy);
        setFont(sub, "800"); ctx.fillStyle = C.muted; ctx.fillText(L.ft, x, cy + sub * 0.7 + 2);
        return;
      }
      // scheduled
      if (!time) return;
      setFont(prim, "800"); ctx.fillStyle = C.eyebrowTime; ctx.fillText(time, x, cy);
    }
    ...
    function drawRow(f, y, S) {
      ...
      var cy = y + S.rowH / 2;
      // Time on the left rail; the matchup is centred in the space to its right.
      drawTimeRail(f, x + 22, cy, S);
      var railRight = x + S.rowTimeCol, cx = (railRight + (x + w)) / 2;
      ...
      var nameLeft = railRight + 8, hRight = hcx - S.crest / 2 - 14;
