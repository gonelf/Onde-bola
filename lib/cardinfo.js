...
function extractCard(fmid, data) {
  ...
  // Single discriminator the share card renders from, so the kickoff time is
  // never placed between the teams (where it reads as a score): scheduled shows
  // "VS" + a time eyebrow, live/finished show the score.
  const state = finished ? "finished" : started ? "live" : "scheduled";
  ...
  return {
    ...
    // state + kickoff HH:MM, kept separate from `status` so the time can live in
    // an eyebrow above the matchup rather than in the centre slot.
    state, time: timeLabel,
    // The live minute / FT reason on its own, for the eyebrow (without the time).
    minute: finished ? reason || "FT" : started ? live || "LIVE" : "",
    date, isoDate,
