...
import { FREE_TO_AIR, FREE_TO_AIR_NORM } from "./broadcasters";
import { DEFAULT_BRAND, brandForOrigin, brandForHost, brandWordmark } from "./brand";
...
const IMAGE_FORMATS = [
  ...
  { id: "square", label: "Quadrado (1080×1080)", maxN: 7, nOpts: [2, 3, 4, 5, 6, 7] },
...
const DIGEST_COPY = {
  en: {
    ...
    title: (h, brand) => `${h} — where to watch on TV · ${brand}`,
  ...
  pt: {
    ...
    title: (h, brand) => `${h} — onde ver na TV · ${brand}`,
...
const IMAGE_COPY = {
  en: {
    ...
    title: "Download an image of the day's games",
  ...
  pt: {
    ...
    title: "Descarregar imagem dos jogos do dia",
...
export async function buildToday({ origin, lang, date, n }) {
  ...
  const brandName = brandForOrigin(origin).name;
  ...
  const title = c.title(headline, brandName);
  ...
  const bodyHtml = `<div class="wrap">
    <header class="site"><span>⚽</span> <a href="/">${esc(brandName)}</a></header>
    ...
    <footer>
      <a href="/">${esc(brandName)}</a> — ${esc(c.footer)}
    </footer>
  </div>`;

  return {
    robots: "index, follow, max-image-preview:large",
    title, description, headline, siteName: brandName,
...
export function buildImage({ date, n, format, highlight, lang, host }) {
  const ic = IMAGE_COPY[lang] || IMAGE_COPY.en;
  const brand = host ? brandForHost(host) : DEFAULT_BRAND;
  ...
  const bodyHtml = `<div class="wrap">
    ...
    <footer><a href="/">${esc(brandName)}</a> — ${esc(ic.footer)}</footer>
  </div>`;
  ...
  (function () {
    ...
    var WM = ${JSON.stringify(wm)};
    var BRAND_FILE = ${JSON.stringify(brand.key)};
    ...
    var FREE_NORM = ${JSON.stringify(FREE_TO_AIR_NORM)};
    var TCOPY = ${JSON.stringify({
      pt: { headToday: "⚽ Jogos de hoje! 🏆", headDay: "⚽ Jogos do dia! 🏆", vs: "vs", live: "(a decorrer)", ht: "(intervalo)", ft: "(terminado)", at: "às", on: "na", footer: "Vê todos os jogos e onde ver na TV 👉 " + brandDomain, noGames: "Sem jogos de relevo para mostrar." },
      en: { headToday: "⚽ Today's games! 🏆", headDay: "⚽ The day's games! 🏆", vs: "vs", live: "(live)", ht: "(half-time)", ft: "(full-time)", at: "at", on: "on", footer: "See every game and where to watch on TV 👉 " + brandDomain, noGames: "No major games to show." },
    })};
    ...
    // Free-to-air by exact name OR normalised key (spacing / HD-tag tolerant),
    // matching lib/broadcasters isPaidChannel — so open channels are recognised
    // and prioritised over paid cable.
    function normCh(s) { return String(s == null ? "" : s).toLowerCase().normalize("NFD").replace(/[\\u0300-\\u036f]/g, "").replace(/\\b(?:hd|uhd|fhd|sd|4k)\\b/g, "").replace(/[^a-z0-9]+/g, ""); }
    function isPaid(ch) { return FREE[ch] ? false : !FREE_NORM[normCh(ch)]; }
    ...
    // A small padlock glyph for the paid channel pill (matches the website's 🔒).
    function drawLock(cx, cy, size, color) {
      var bw = size * 0.74, bh = size * 0.52, by = cy - bh * 0.05;
      ctx.fillStyle = color;
      roundRect(cx - bw / 2, by, bw, bh, size * 0.12); ctx.fill();
      ctx.lineWidth = Math.max(1.2, size * 0.12); ctx.strokeStyle = color;
      var r = bw * 0.34;
      ctx.beginPath(); ctx.arc(cx, by + size * 0.02, r, Math.PI, 0); ctx.stroke();
    }
    // Width of the channel pill at font size fs (so a centred group — e.g. the
    // hero's time + pill — can be measured before drawing).
    function pillWidth(ch, paid, fs, maxW) {
      var padH = Math.round(fs * 0.5), lockW = paid ? fs * 0.78 : 0, lockGap = paid ? fs * 0.26 : 0;
      setFont(fs, "700");
      var label = ellipsize(ch, maxW - padH * 2 - lockW - lockGap - 2);
      return padH * 2 + lockW + lockGap + ctx.measureText(label).width;
    }
    // The channel pill, styled like the website's chips: rounded, soft tinted
    // fill + border; free-to-air green, paid amber with a padlock. Left-anchored
    // at x, vertically centred on cy.
    function drawChannelPill(x, cy, ch, paid, fs, maxW) {
      var fg = paid ? "#f5b041" : C.accent;
      var bg = paid ? "rgba(245,176,65,0.14)" : "rgba(22,210,122,0.14)";
      var bd = paid ? "rgba(245,176,65,0.3)" : "rgba(22,210,122,0.25)";
      var padH = Math.round(fs * 0.5), padV = Math.round(fs * 0.24);
      var lockW = paid ? fs * 0.78 : 0, lockGap = paid ? fs * 0.26 : 0;
      setFont(fs, "700");
      var label = ellipsize(ch, maxW - padH * 2 - lockW - lockGap - 2);
      var pillW = padH * 2 + lockW + lockGap + ctx.measureText(label).width, pillH = fs + padV * 2;
      roundRect(x, cy - pillH / 2, pillW, pillH, Math.round(fs * 0.5));
      ctx.fillStyle = bg; ctx.fill();
      ctx.lineWidth = 1; ctx.strokeStyle = bd; ctx.stroke();
      var tx = x + padH;
      if (paid) { drawLock(tx + lockW / 2, cy, lockW, fg); tx += lockW + lockGap; }
      setFont(fs, "700"); ctx.fillStyle = fg; ctx.textAlign = "left"; ctx.textBaseline = "middle";
    ...
    // Width drawEyebrow occupies, so the hero can centre "time + channel pill"
    // as one group. Mirrors the geometry in drawEyebrow.
    function eyebrowWidth(f, timeFont, labelFont) {
      var state = matchState(f), time = fmtTime(f.kickoff);
      if (state === "live") {
        var minute = (f.status || L.live); if (minute.length > 6) minute = minute.slice(0, 6);
        setFont(timeFont, "800"); return timeFont * 0.22 * 2 + timeFont * 0.34 + ctx.measureText(minute).width;
      }
      if (state === "finished") {
        if (!time) { setFont(labelFont, "800"); return ctx.measureText(L.ft).width; }
        setFont(timeFont, "800"); var tw = ctx.measureText(time).width;
        setFont(labelFont, "800"); return tw + labelFont * 0.5 + ctx.measureText("· " + L.ft).width;
      }
      if (!time) return 0;
      setFont(timeFont, "800"); return timeFont * 0.46 * 2 + timeFont * 0.4 + ctx.measureText(time).width;
    }
    ...
    function drawRail(f, x, cy, S) {
      ...
      if (ch) {
        var paid = isPaid(ch);
        lines.push({ h: S.railChannel + Math.round(S.railChannel * 0.24) * 2, draw: function (yc) {
          drawChannelPill(x, yc, ch, paid, S.railChannel, S.railCol);
        } });
      }
    ...
    function drawHero(f, y, S) {
      ...
      // Time banner (eyebrow) directly above the matchup, with the channel pill
      // to the right of the kickoff time — the whole group centred on cx.
      var eyeY = y + 24 + S.heroCompFont + 14 + S.heroEyebrowFont / 2;
      var hch = ptChannelFor(f);
      var eyeW = eyebrowWidth(f, S.heroEyebrowFont, S.heroEyebrowLabelFont);
      if (hch) {
        var pgap = Math.round(S.heroEyebrowLabelFont * 0.7);
        var pw = pillWidth(hch, isPaid(hch), S.heroEyebrowLabelFont, w * 0.5);
        var groupX = cx - (eyeW + pgap + pw) / 2;
        drawEyebrow(f, groupX + eyeW / 2, eyeY, S.heroEyebrowFont, S.heroEyebrowLabelFont);
        drawChannelPill(groupX + eyeW + pgap, eyeY, hch, isPaid(hch), S.heroEyebrowLabelFont, w * 0.5);
      } else {
    ...
    function draw(hero, top, S, isToday, d) {
      ...
      var wmHead = WM.head ? WM.head + " " : "";
      ctx.fillStyle = C.text; ctx.fillText(wmHead, bx, brandY);
      ctx.fillStyle = C.accent; ctx.fillText(WM.tail, bx + ctx.measureText(wmHead).width, brandY);
    ...
    function download() {
      ...
      canvas.toBlob(function (blob) {
        ...
        a.download = BRAND_FILE + "-" + d + "-" + fmtEl.value + ".png";
    ...
    function copyText() {
      ...
      function fallback() {
        try {
          textEl.focus();
          textEl.select();
          if (textEl.setSelectionRange) textEl.setSelectionRange(0, v.length); // iOS/webviews ignore select() on readonly
