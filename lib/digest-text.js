...
async function getJson(url, ms, auth) {
...
// Exported so the reel props builder (scripts/reel-cron.mjs) shows the same
// channel per game as this caption.
export function ptChannelFor(f, fmtvFeed, rich) {
...
/**
 * Caption for the daily animated-replay post of ONE finished game (the video
 * sibling of the digest caption above). Flags resolve for national sides and
 * degrade to nothing for clubs, like the game lines.
 *   game  { home, away, homeScore, awayScore, competition }
 *   lang  "pt" (default) or "en"
 *   site  the brand's public domain, for the footer
 */
export function buildMatchCaption({ game, lang, site }) {
  const c = COPY[lang] || COPY.pt;
  const hf = flagForTeam(game.home), af = flagForTeam(game.away);
  const home = (hf ? hf + " " : "") + game.home;
  const away = (af ? af + " " : "") + game.away;
  const score = `${game.homeScore ?? "?"}-${game.awayScore ?? "?"}`;
  const comp = game.competition ? ` · ${game.competition}` : "";
  const header = lang === "en" ? "🎬 The match in one minute! ⚽" : "🎬 O filme do jogo! ⚽";
  return `${header}\n\n${home} ${score} ${away}${comp}\n\n${c.footer(site)}\n`;
}
 ...
 *   lang    — "pt" (default) or "en"
 *   auth    — headers from forwardAuthHeaders() so the internal feeds survive
 *             Deployment Protection on preview deployments (else they wall off
 *             and the post reports "no games" even on a day full of fixtures)
 */
export async function buildTextPost({ origin, date, n, lang, auth }) {
  const c = COPY[lang] || COPY.pt;
  const day = /^\d{4}-\d{2}-\d{2}$/.test(date || "") ? date : todayYmd();
  const isToday = day === todayYmd();

  const [fx, listings, fmtv] = await Promise.all([
    getJson(`${origin}/api/fixtures?date=${day}&all=1`, 6000, auth),
    getJson(`${origin}/api/listings?date=${day}`, 5000, auth),
    getJson(`${origin}/api/fmtv?date=${day}`, 5000, auth),
  ]);
