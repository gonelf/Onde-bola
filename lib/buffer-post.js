...
// Which channels to post to is managed from /admin/buffer (stored in KV); the
// env var is an ops fallback, and failing both we auto-default to the account's
// Facebook / Instagram / X channels.
const CHANNELS_KEY = "buffer:channels";
...
// The channel ids saved from the admin page (empty when never set).
...
// Persist the admin's channel selection (de-duplicated). An explicit empty list
// is honoured (clears the saved selection, falling back to env/auto-default).
export async function saveStoredChannelIds(ids) {
  const clean = Array.from(new Set(
    (Array.isArray(ids) ? ids : []).map((x) => String(x).trim()).filter(Boolean)
  ));
  await kv(["SET", CHANNELS_KEY, JSON.stringify(clean)]);
  return clean;
}
