...
// ---- post-game ("what happened") helpers --------------------------------

// Pull a {home,away} goal tally out of a "2 - 1"-style score string.
function parseScore(score) {
  const m = /(\d+)\s*[-–:]\s*(\d+)/.exec(String(score || ""));
  return m ? { home: Number(m[1]), away: Number(m[2]) } : null;
}

// matchdetails event kinds that count as a goal on the scoreboard.
const GOAL_KINDS = { goal: 1, pengoal: 1, owngoal: 1 };
const EVENT_ICON = { goal: "⚽", pengoal: "⚽", owngoal: "⚽", yellow: "🟨", red: "🟥", sub: "🔁" };

// "Gonçalves (23', pen.)" — name plus minute and a pen/own-goal marker.
function scorerLabel(c, ev) {
  const marks = [];
  if (ev.min) marks.push(ev.min);
  if (ev.kind === "pengoal") marks.push(c.markPen);
  else if (ev.kind === "owngoal") marks.push(c.markOg);
  return `${ev.player}${marks.length ? " (" + marks.join(", ") + ")" : ""}`;
}

// Split goal events into per-team scorer lists (own goals credited to the team
// that benefited) plus a flat chronological list for the FAQ.
function goalScorers(c, events) {
  const home = [], away = [], all = [];
  (Array.isArray(events) ? events : []).forEach((ev) => {
    if (!ev || !GOAL_KINDS[ev.kind] || !ev.player) return;
    const benefit = ev.kind === "owngoal"
      ? (ev.side === "home" ? "away" : ev.side === "away" ? "home" : "")
      : ev.side;
    const label = scorerLabel(c, ev);
    all.push(label);
    if (benefit === "home") home.push(label);
    else if (benefit === "away") away.push(label);
  });
  return { home, away, all };
}

// Phase 2: a templated natural-language match report from the score + timeline.
function buildReport(c, { home, away, score, parsed, comp, venue, events, scorers }) {
  if (!parsed) return "";
  let sentence;
  if (parsed.home > parsed.away) sentence = c.reportWin(home, away, score);
  else if (parsed.away > parsed.home) sentence = c.reportWin(away, home, score);
  else sentence = c.reportDraw(home, away, score);
  sentence += (comp ? c.reportIn(comp) : "") + (venue ? c.reportAt(venue) : "") + ".";

  const out = [sentence];
  const sc = c.reportScorers(home, scorers.home.join(", "), away, scorers.away.join(", "));
  if (sc) out.push(sc);
  else if (parsed.home === 0 && parsed.away === 0) out.push(c.reportNoGoals);

  const reds = (Array.isArray(events) ? events : [])
    .filter((ev) => ev && ev.kind === "red" && ev.player)
    .map((ev) => `${ev.player}${ev.min ? " (" + ev.min + ")" : ""}`);
  if (reds.length) out.push(c.reportRed(reds.join(", ")));

  return out.join(" ");
}

// One stacked home/away bar for a single match stat (possession, shots, …).
function statBarHtml(c, row) {
  const labels = {
    possession: c.statPossession, xg: c.statXg, shots: c.statShots,
    sot: c.statSot, corners: c.statCorners, fouls: c.statFouls,
  };
  const name = labels[row.key] || row.key;
  const hv = parseFloat(String(row.home).replace(/[^\d.]/g, "")) || 0;
  const av = parseFloat(String(row.away).replace(/[^\d.]/g, "")) || 0;
  const total = hv + av;
  const hp = total ? Math.round((hv / total) * 100) : 50;
  return `<div class="stat">
    <div class="lab"><span>${esc(row.home)}</span><span>${esc(name)}</span><span>${esc(row.away)}</span></div>
    <div class="bar"><span class="h" style="width:${hp}%"></span><span class="a" style="width:${100 - hp}%"></span></div>
  </div>`;
}

// One timeline row (goal/card), mirrored to the right for away-team events.
function timelineLi(c, ev) {
  const tag = ev.kind === "owngoal" ? ` (${c.markOg})` : ev.kind === "pengoal" ? ` (${c.markPen})` : "";
  const note = ev.note ? ` (${ev.note})` : "";
  const side = ev.side === "away" ? "away" : ev.side === "home" ? "home" : "n";
  return `<li class="ev-${side}">
    <span class="min">${esc(ev.min)}</span>
    <span class="ico">${EVENT_ICON[ev.kind] || ""}</span>
    <span class="who">${esc(ev.player)}${esc(tag)}${esc(note)}</span>
  </li>`;
}
...
const COPY = {
  en: {
    ...
    headingFinished: (vs) => `${vs}: result, goals & highlights`,
    titleFinished: (vs, comp, score) =>
      `${vs}${score ? " " + score : ""} result, goals & highlights${comp ? " — " + comp : ""} · Hoje Há Bola`,
    descFinished: "Final score, goal timeline, match stats and highlights.",
    whatHappened: "What happened",
    timelineHeading: "Match timeline",
    statsHeading: "Match stats",
    motmLabel: "Player of the match",
    highlightsHeading: (vs) => `${vs} highlights`,
    highlightsWatch: (vs) => `Watch ${vs} highlights →`,
    statPossession: "Possession", statXg: "Expected goals (xG)", statShots: "Shots",
    statSot: "Shots on target", statCorners: "Corners", statFouls: "Fouls",
    markPen: "pen.", markOg: "o.g.",
    // report builder
    reportWin: (w, l, score) => `${w} beat ${l} ${score}`,
    reportDraw: (h, a, score) => `${h} and ${a} drew ${score}`,
    reportIn: (comp) => ` in the ${comp}`,
    reportAt: (venue) => ` at ${venue}`,
    reportScorers: (ht, hn, at, an) => {
      if (hn && an) return `${hn} scored for ${ht}, while ${an} replied for ${at}.`;
      if (hn) return `${hn} scored for ${ht}.`;
      if (an) return `${an} scored for ${at}.`;
      return "";
    },
    reportNoGoals: "Neither side could find the net.",
    reportRed: (n) => `Sent off: ${n}.`,
    // post-game FAQ
    faqScoreQ: (vs) => `What was the final score of ${vs}?`,
    faqScoreA: (vs, score, comp) => `${vs} finished ${score}${comp ? " in the " + comp : ""}.`,
    faqScorersQ: (vs) => `Who scored in ${vs}?`,
    faqScorersA: (scorers) => `Goalscorers: ${scorers}.`,
    faqScorersNoA: (vs) => `There were no goals in ${vs}.`,
    faqHighlightsQ: (vs) => `Where can I watch ${vs} highlights?`,
    faqHighlightsA: (vs) => `Highlights of ${vs} are above, and broadcaster replays are listed by country.`,
    // player ratings child page
    ratingsCrumb: "Player ratings",
    ratingsTitle: (vs, comp) => `${vs} player ratings${comp ? " — " + comp : ""} · Hoje Há Bola`,
    ratingsHeading: (vs) => `${vs} player ratings`,
    ratingsDesc: (vs) => `Player ratings for ${vs} — every starter scored out of 10, plus the player of the match.`,
    ratingsLead: (vs) => `How every player performed in ${vs}, rated out of 10.`,
    ratingsMotm: "Player of the match",
    ratingsBackToReport: "← Full match report",
    ratingsLinkFromHub: "Player ratings →",
    ratingsNoData: (vs) => `Player ratings for ${vs} aren't available yet — check back after full time.`,
  ...
  pt: {
    ...
    // pós-jogo ("o que aconteceu")
    headingFinished: (vs) => `${vs}: resultado, golos e resumo`,
    titleFinished: (vs, comp, score) =>
      `${vs}${score ? " " + score : ""} resultado, golos e resumo${comp ? " — " + comp : ""} · Hoje Há Bola`,
    descFinished: "Resultado final, cronologia dos golos, estatísticas e resumo em vídeo.",
    whatHappened: "O que aconteceu",
    timelineHeading: "Cronologia do jogo",
    statsHeading: "Estatísticas do jogo",
    motmLabel: "Homem do jogo",
    highlightsHeading: (vs) => `Resumo de ${vs}`,
    highlightsWatch: (vs) => `Ver o resumo de ${vs} →`,
    statPossession: "Posse de bola", statXg: "Golos esperados (xG)", statShots: "Remates",
    statSot: "Remates à baliza", statCorners: "Cantos", statFouls: "Faltas",
    markPen: "g.p.", markOg: "a.g.",
    // construção do resumo
    reportWin: (w, l, score) => `${w} venceu ${l} por ${score}`,
    reportDraw: (h, a, score) => `${h} e ${a} empataram ${score}`,
    reportIn: (comp) => ` na ${comp}`,
    reportAt: (venue) => ` em ${venue}`,
    reportScorers: (ht, hn, at, an) => {
      const parts = [];
      if (hn) parts.push(`${ht}: ${hn}`);
      if (an) parts.push(`${at}: ${an}`);
      return parts.length ? `Marcadores — ${parts.join("; ")}.` : "";
    },
    reportNoGoals: "Nenhuma equipa conseguiu marcar.",
    reportRed: (n) => `Expulsão: ${n}.`,
    // FAQ pós-jogo
    faqScoreQ: (vs) => `Qual foi o resultado final de ${vs}?`,
    faqScoreA: (vs, score, comp) => `${vs} terminou ${score}${comp ? " na " + comp : ""}.`,
    faqScorersQ: (vs) => `Quem marcou em ${vs}?`,
    faqScorersA: (scorers) => `Marcadores: ${scorers}.`,
    faqScorersNoA: (vs) => `Não houve golos em ${vs}.`,
    faqHighlightsQ: (vs) => `Onde posso ver o resumo de ${vs}?`,
    faqHighlightsA: (vs) => `O resumo de ${vs} está acima, e as repetições dos canais estão listadas por país.`,
    // página de avaliações dos jogadores
    ratingsCrumb: "Avaliações dos jogadores",
    ratingsTitle: (vs, comp) => `Avaliações dos jogadores de ${vs}${comp ? " — " + comp : ""} · Hoje Há Bola`,
    ratingsHeading: (vs) => `Avaliações dos jogadores de ${vs}`,
    ratingsDesc: (vs) => `Avaliações dos jogadores de ${vs} — cada titular pontuado de 0 a 10, mais o homem do jogo.`,
    ratingsLead: (vs) => `O desempenho de cada jogador em ${vs}, avaliado de 0 a 10.`,
    ratingsMotm: "Homem do jogo",
    ratingsBackToReport: "← Resumo completo do jogo",
    ratingsLinkFromHub: "Avaliações dos jogadores →",
    ratingsNoData: (vs) => `As avaliações dos jogadores de ${vs} ainda não estão disponíveis — volta após o final.`,
...
const SHARE_CSS = `
  ...
  *{box-sizing:border-box}
  ...
  .report{margin:0 0 6px}
  .motm{margin:12px 0 0;color:var(--muted)}
  .timeline{list-style:none;padding:0;margin:8px 0 0}
  .timeline li{display:flex;align-items:baseline;gap:10px;padding:7px 0;border-bottom:1px solid var(--line);font-size:.92rem}
  .timeline li:last-child{border-bottom:0}
  .timeline .min{color:var(--muted);min-width:42px;font-variant-numeric:tabular-nums}
  .timeline .ico{min-width:1.3em;text-align:center}
  .timeline .who{flex:1}
  .timeline .ev-away{flex-direction:row-reverse;text-align:right}
  .stat{margin:12px 0}
  .stat .lab{display:flex;justify-content:space-between;gap:8px;font-size:.85rem;color:var(--muted);margin-bottom:5px}
  .stat .lab span:first-child,.stat .lab span:last-child{color:var(--txt);font-weight:600;font-variant-numeric:tabular-nums}
  .stat .bar{display:flex;height:6px;border-radius:3px;overflow:hidden;background:#0f1a26}
  .stat .bar .h{background:var(--accent)}
  .stat .bar .a{background:#e0a23b}
  .hl{position:relative;padding-bottom:56.25%;height:0;border-radius:10px;overflow:hidden;background:#0f1a26}
  .hl iframe{position:absolute;top:0;left:0;width:100%;height:100%;border:0}
`;
...
export async function buildShare({ origin, lang, parts, query, auth }) {
  ...
  // Post-game data, derived from matchdetails (events/stats/highlights/motm).
  // A finished match shifts the page from "where to watch" to "what happened".
  const events = (details && Array.isArray(details.events)) ? details.events : [];
  const stats = (details && Array.isArray(details.stats)) ? details.stats : [];
  const motm = (details && details.motm) || null;
  const highlights = (details && details.highlights) || null;
  const hasHighlights = !!(highlights && (highlights.youtubeId || highlights.url));
  const parsedScore = finished ? parseScore(score) : null;
  const scorers = finished ? goalScorers(c, events) : { home: [], away: [], all: [] };
  const report = finished
    ? buildReport(c, {
        home, away, score, parsed: parsedScore,
        comp: compDisplay, venue: details && details.venue,
        events, scorers,
      })
    : "";

  // Once finished, the headline/title/description lead with the result.
  const heading = finished ? c.headingFinished(vs) : c.heading(vs);
  const headline =
    vs + (finished && score ? " " + score : "") + (compDisplay ? " — " + compDisplay : "");
  const title = finished ? c.titleFinished(vs, compDisplay, score) : c.title(vs, compDisplay);
  const description = finished
    ? `${vs}${score ? " " + score : ""}${compDisplay ? " · " + compDisplay : ""}. ${c.descFinished}`
    : `${vs}${compDisplay ? " · " + compDisplay : ""}${when ? " · " + when : ""}. ${c.descSentence}`;
  ...
  // A highlights clip on a finished match → a VideoObject. uploadDate is
  // required for valid markup, so only emit it when we have a date to use.
  const videoDate = kickoff || (isoDate ? `${isoDate}T00:00:00Z` : "");
  if (finished && hasHighlights && videoDate) {
    const yt = highlights.youtubeId;
    const video = {
      "@type": "VideoObject",
      name: c.highlightsHeading(vs),
      description,
      thumbnailUrl: yt ? `https://i.ytimg.com/vi/${yt}/hqdefault.jpg` : imageUrl,
      uploadDate: videoDate,
    };
    if (yt) video.embedUrl = `https://www.youtube.com/embed/${yt}`;
    if (highlights.url) video.contentUrl = highlights.url;
    graph.push(video);
  }
  ...
  introBits.push(
    finished
      // The "What happened" report below already leads with the final score; only
      // fall back to the one-line introFinal when we couldn't build a report.
      ? (report ? "" : score ? c.introFinal(score) : "")
      : freeAny ? c.introFree : c.introPaid
  );
  const intro = introBits.filter(Boolean).join(" ");
  ...
  // Link to the player-ratings child page once the match is over and FotMob has
  // rated the XI — gives the child page an internal link so it gets crawled.
  const ratedStarters = []
    .concat(lineups && lineups.home && lineups.home.starters ? lineups.home.starters : [])
    .concat(lineups && lineups.away && lineups.away.starters ? lineups.away.starters : []);
  const ratingsLink = (finished && ratedStarters.some((p) => p && p.rating))
    ? `<p class="more"><a href="${esc(shareUrl)}/player-ratings">${esc(c.ratingsLinkFromHub)}</a></p>`
    : "";
  ...
  // --- post-game sections (finished matches only) ---
  const timelineEvents = finished
    ? events.filter((ev) => ev && ev.kind !== "sub" && (ev.player || ev.min))
    : [];
  const motmHtml = (finished && motm && motm.name)
    ? `<p class="motm"><strong>${esc(c.motmLabel)}:</strong> ${esc(motm.name)}${motm.rating ? ` (${esc(motm.rating)})` : ""}</p>`
    : "";
  const whatHappenedSection = (finished && (report || timelineEvents.length || motmHtml))
    ? `<section class="card">
    <h2>${esc(c.whatHappened)}</h2>
    ${report ? `<p class="report">${esc(report)}</p>` : ""}
    ${timelineEvents.length ? `<h3>${esc(c.timelineHeading)}</h3>
    <ul class="timeline">${timelineEvents.map((ev) => timelineLi(c, ev)).join("")}</ul>` : ""}
    ${motmHtml}
  </section>`
    : "";

  const statsSection = (finished && stats.length)
    ? `<section class="card">
    <h2>${esc(c.statsHeading)}</h2>
    ${stats.map((r) => statBarHtml(c, r)).join("")}
  </section>`
    : "";

  const highlightsSection = (finished && hasHighlights)
    ? `<section class="card">
    <h2>${esc(c.highlightsHeading(vs))}</h2>
    ${highlights.youtubeId
        ? `<div class="hl"><iframe src="https://www.youtube-nocookie.com/embed/${esc(highlights.youtubeId)}" title="${esc(c.highlightsHeading(vs))}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`
        : `<p><a href="${esc(highlights.url)}" target="_blank" rel="noopener">${esc(c.highlightsWatch(vs))}</a></p>`}
  </section>`
    : "";

  const faqs = [];
  // Finished matches lead with result-oriented questions.
  if (finished) {
    if (score) faqs.push([c.faqScoreQ(vs), c.faqScoreA(vs, score, compDisplay)]);
    faqs.push(scorers.all.length
      ? [c.faqScorersQ(vs), c.faqScorersA(scorers.all.join(", "))]
      : [c.faqScorersQ(vs), c.faqScorersNoA(vs)]);
    if (hasHighlights) faqs.push([c.faqHighlightsQ(vs), c.faqHighlightsA(vs)]);
  }
  if (primaryTv) {
    faqs.push([
...
// ---- Player ratings child page (/g/<…>/player-ratings) -----------------

// Shares the per-game look, plus the rating list + colour-coded score chips.
const RATINGS_CSS = SHARE_CSS + `
  .rt{list-style:none;padding:0;margin:0}
  .rt li{display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--line)}
  .rt li:last-child{border-bottom:0}
  .rt .num{min-width:1.6em;color:var(--muted);font-variant-numeric:tabular-nums}
  .rt .nm{flex:1;font-weight:600}
  .rt .pos{color:var(--muted);font-size:.78rem;text-transform:uppercase;letter-spacing:.03em}
  .rt .rating{min-width:44px;text-align:center;font-weight:800;border-radius:6px;padding:3px 8px;font-variant-numeric:tabular-nums}
  .rt-hi{background:rgba(22,210,122,.20);color:var(--accent)}
  .rt-mid{background:rgba(22,210,122,.11);color:var(--accent)}
  .rt-ok{background:#0f1a26;color:var(--txt)}
  .rt-lo{background:rgba(224,162,59,.16);color:#e0a23b}
  .rt-na{background:#0f1a26;color:var(--muted)}
`;

// `parts` is the /g/* path with the trailing "player-ratings" segment already
// stripped, so it resolves the same match the hub page does.
export async function buildRatings({ origin, lang, parts, query, auth }) {
  parts = parts || [];
  query = query || {};
  const c = COPY[lang] || COPY.en;
  const get = (k) => (query[k] == null ? "" : String(query[k]));
  ...
  let resolvedFx = null;
  if (slug && pathDate) resolvedFx = await resolveSlug(origin, pathDate, slug, auth).catch(() => null);
  const slugNames = slug ? parseSlug(slug) : null;
  ...
  let card = null;
  if (hasId) { const r = await getCard(fmid).catch(() => null); if (r && r.ok) card = r.card; }
  ...
  let details = null;
  if (hasId) {
    const md = await getJson(`${origin}/api/matchdetails?id=${fmid}`, 4500, { headers: auth });
    if (md && md.ok && md.details) details = md.details;
  }
  ...
  const hubUrl = isoDate
    ? (leagueSlug
        ? `${origin}/g/${leagueSlug}/${isoDate}/${matchSlug(home, away)}`
        : `${origin}/g/${isoDate}/${matchSlug(home, away)}`)
    : `${origin}/g/${fmid}`;
  const canonical = `${hubUrl}/player-ratings`;

  const vs = `${home} vs ${away}`;
  const imageUrl = hasId ? `${origin}/og/${fmid}` : "";

  const lineups = details && details.lineups;
  const motm = details && details.motm;
  const starters = (s) => (s && Array.isArray(s.starters) ? s.starters : []);
  const homeXi = starters(lineups && lineups.home);
  const awayXi = starters(lineups && lineups.away);
  const hasRatings = [].concat(homeXi, awayXi).some((p) => p && p.rating);

  // Ratings only exist after the match, so an unrated page stays out of the index.
  const indexable = finished && isNotable(comp) && hasRatings;
  const robots = indexable ? "index, follow, max-image-preview:large" : "noindex, follow";

  const title = c.ratingsTitle(vs, compDisplay);
  const description = c.ratingsDesc(vs);
  const headline = c.ratingsHeading(vs);

  const ratingChip = (r) => {
    if (!r) return `<span class="rating rt-na">–</span>`;
    const n = parseFloat(r);
    const cls = n >= 8 ? "rt-hi" : n >= 7 ? "rt-mid" : n >= 6 ? "rt-ok" : "rt-lo";
    return `<span class="rating ${cls}">${esc(r)}</span>`;
  };
  const teamList = (xi) => {
    const sorted = xi.slice().sort((a, b) => (parseFloat(b.rating) || 0) - (parseFloat(a.rating) || 0));
    return `<ul class="rt">${sorted.map((p) => `<li>
      ${p.num ? `<span class="num">${esc(p.num)}</span>` : ""}
  ...
  const teamsHtml = hasRatings
    ? `<section class="card"><h2>${esc(home)}</h2>${teamList(homeXi)}</section>
    <section class="card"><h2>${esc(away)}</h2>${teamList(awayXi)}</section>`
    : `<section class="card"><p>${esc(c.ratingsNoData(vs))}</p></section>`;

  const motmHtml = (hasRatings && motm && motm.name)
    ? `<section class="card"><h2>${esc(c.ratingsMotm)}</h2>
    <p><strong>${esc(motm.name)}</strong>${motm.rating ? ` — ${esc(motm.rating)}` : ""}</p></section>`
    : "";

  const crumbs = [{ name: "Hoje Há Bola", item: origin + "/" }];
  if (comp && leagueUrl) crumbs.push({ name: compDisplay, item: leagueUrl });
  crumbs.push({ name: vs, item: hubUrl });
  crumbs.push({ name: c.ratingsCrumb, item: canonical });
  const breadcrumb = {
    "@type": "BreadcrumbList",
    itemListElement: crumbs.map((cc, i) => ({
      "@type": "ListItem", position: i + 1, name: cc.name, item: cc.item,
    })),
  };
  const jsonLd = JSON.stringify({ "@context": "https://schema.org", "@graph": [breadcrumb] });

  const bodyHtml = `<div class="wrap">
    <header class="site"><span>⚽</span> <a href="/">Hoje Há Bola</a></header>
    <nav class="crumbs" aria-label="Breadcrumb">
      <a href="/">${esc(c.crumbHome)}</a>${comp && leagueUrl ? ` › <a href="${esc(leagueUrl)}">${esc(compDisplay)}</a>` : ""} › <a href="${esc(hubUrl)}">${esc(vs)}</a> › <span>${esc(c.ratingsCrumb)}</span>
    </nav>

    <h1>${esc(c.ratingsHeading(vs))}</h1>
    <p class="meta">${esc([compDisplay, score].filter(Boolean).join(" · "))}</p>
    <p class="intro">${esc(c.ratingsLead(vs))}</p>

    ${motmHtml}
    ${teamsHtml}

    <p class="more"><a href="${esc(hubUrl)}">${esc(c.ratingsBackToReport)}</a></p>
  ...
  return {
    ok: true,
    robots, title, description, headline,
    canonical, ogType: "article", ogImage: imageUrl,
    css: RATINGS_CSS, bodyHtml, jsonLd,
  };
}
