...
export const matchResults = pgTable("match_results", {
  ...
  // { home:{name,formation,color}, away:{...}, seed } — the team descriptors the
  // replay viewer hands to <MatchPitch>, so a result renders without extra joins.
...
// --- Durable app config (Supabase migration Phase 2) ------------------------
// These mirror the admin-managed settings + the Buffer automation that currently
// live in Upstash KV. The schema + migration land first; each store in lib/*
// switches its reads/writes here once Supabase is wired up (DATABASE_URL set in
// every environment). See docs/supabase-migration.md.

// Feature flags (lib/flags.js): one row per flag override (id → state). Flags
// with no row fall back to their built-in default in FLAG_DEFS.
...
// Ad units (lib/ads-store.js): one row per injected unit. `position` preserves
// the authored order; `banner` holds the {key,width,height,format} shape.
export const adUnits = pgTable("ad_units", {
  ...
  everyN: integer("every_n").notNull().default(5),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// Manual TV overrides (lib/overrides.js): one row per match (FotMob id), the
// per-country broadcaster rows as jsonb — the highest-trust listings source.
export const tvOverrides = pgTable("tv_overrides", {
  fmid: text("fmid").primaryKey(),
  date: text("date"),
  home: text("home"),
  away: text("away"),
...
// Match-replay animation defaults (lib/replay-config.js): a single row (id=1)
// holding the tunables + display/sound/audio toggles as jsonb.
export const replayConfig = pgTable("replay_config", {
  id: integer("id").primaryKey().default(1),
  cfg: jsonb("cfg"),
  display: jsonb("display"),
  eventSounds: jsonb("event_sounds"),
  audio: jsonb("audio"),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// Buffer automation (lib/buffer-post.js):
//   buffer_channels — which channels to post to (one row per channel id).
//   buffer_log      — append-only history of schedule attempts; replaces the
//                     capped JSON blob (no 50-row cap, no read-modify-write race,
//                     queryable by date/status/trigger).
export const bufferChannels = pgTable("buffer_channels", {
  channelId: text("channel_id").primaryKey(),
  position: integer("position").notNull().default(0),
  addedAt: timestamp("added_at").defaultNow().notNull(),
});

export const bufferLog = pgTable("buffer_log", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
...
// Canonical SEO URL registry (lib/sitemap-sweep): one row per URL with its
// lastmod (YYYY-MM-DD). Backs /sitemap.xml and keeps known /g pages indexable.
// Replaces the seo:urls KV hash; gains real prune-by-date queries.
export const seoUrls = pgTable("seo_urls", {
