...
// Introspect the GraphQL schema for the createPost input type, so we can see the
// real field names (e.g. how media is attached — `imageUrl` was rejected as
// "not defined"). Returns { ok, fields:["name: Type", …], error }.
function gqlTypeName(t) {
  if (!t) return "?";
  return t.name || (t.ofType ? gqlTypeName(t.ofType) : t.kind) || "?";
}
export async function introspectCreatePost() {
  const token = process.env.BUFFER_ACCESS_TOKEN || "";
  if (!token) return { ok: false, fields: [], error: "BUFFER_ACCESS_TOKEN not set" };
  const q = `query { __type(name: "CreatePostInput") { inputFields { name type { kind name ofType { kind name ofType { kind name } } } } } }`;
  const r = await bufferGraphQL(q, token);
  const errs = gqlErrors(r.json);
  if (errs) return { ok: false, fields: [], error: errs };
  const type = r.json && r.json.data && r.json.data.__type;
  const fields = type && type.inputFields;
  if (!Array.isArray(fields)) {
    return { ok: false, fields: [], error: "no schema returned (introspection may be disabled)", raw: typeof r.text === "string" ? r.text.slice(0, 300) : "" };
  }
  return { ok: true, fields: fields.map((f) => `${f.name}: ${gqlTypeName(f.type)}`), error: "" };
}
