...
// The classification (league table) for a competition, keyed by its FotMob
// league id. Session-cached so reopening a competition's table is instant and a
// silent refresh tick doesn't re-fetch. Returns { ok, groups:[{name,rows}] };
// degrades to { ok:false, groups:[] } for cups / internationals with no table.
var standingsCache = {};
export function fetchStandings(leagueId) {
  var id = String(leagueId == null ? "" : leagueId);
  if (!id) return Promise.resolve({ ok: false, groups: [] });
  ...
  return fetch("/api/standings?league=" + encodeURIComponent(id), { headers: { Accept: "application/json" } })
    ...
    .then(function (d) {
      var out = { ok: !!(d && d.ok), name: (d && d.name) || "", groups: (d && d.groups) || [] };
      standingsCache[id] = out;
      return out;
    })
    .catch(function () { return { ok: false, groups: [] }; });
}
