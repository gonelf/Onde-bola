...
// Same competition ranking the digest image uses, so the text picks the same
// marquee games in the same order.
...
function phase(f) {
  ...
  if (s && s !== "FT") return 0; // live (in play)
...
const COPY = {
  pt: {
    ...
    footer: `Vê todos os jogos e onde ver na TV 👉 ${SITE}`,
  ...
  en: {
    ...
    footer: `See every game and where to watch on TV 👉 ${SITE}`,
...
export async function buildTextPost({ origin, date, n, lang }) {
  ...
  const top = sortedTop(fx && fx.fixtures, clampTextN(n));
