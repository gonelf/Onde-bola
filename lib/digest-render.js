...
const IMAGE_FORMATS = [
  { id: "landscape", label: "Paisagem (1200×630)", maxN: 6, nOpts: [1, 2, 3, 4, 5, 6] },
  { id: "square", label: "Quadrado (1080×1080)", maxN: 7, nOpts: [1, 2, 3, 4, 5, 6, 7] },
  { id: "story", label: "Story (1080×1920)", maxN: 12, nOpts: [1, 2, 3, 5, 6, 8, 10, 12] },
];
...
const IMAGE_COPY = {
  en: {
    ...
    labelDate: "Date", labelFormat: "Format", labelN: "Games in image", labelHighlight: "Highlighted",
  ...
  pt: {
    ...
    labelDate: "Data", labelFormat: "Formato", labelN: "Jogos na imagem", labelHighlight: "Em destaque",
...
const CANVAS_FORMATS = {
  landscape: { W: 1200, H: 630, pad: 56, accent: 10, brandDot: 24, brandFont: 28, titleFont: 40, dateFont: 28, tagFont: 20, rowH: 84, rowGap: 10, nameFont: 28, crest: 46, scoreFont: 30, badge: 34, scoreHalf: 78, gap: 16, chanFont: 17, heroH: 262, heroGap: 16, heroCompFont: 22, heroName: 36, heroCrest: 96, heroScore: 64, heroStatusFont: 22, heroBadge: 32, heroScoreHalf: 96, heroChanFont: 22, emptyFont: 40 },
  square: { W: 1080, H: 1080, pad: 64, accent: 12, brandDot: 28, brandFont: 32, titleFont: 48, dateFont: 28, tagFont: 22, rowH: 112, rowGap: 14, nameFont: 32, crest: 56, scoreFont: 34, badge: 40, scoreHalf: 84, gap: 18, chanFont: 21, heroH: 360, heroGap: 20, heroCompFont: 26, heroName: 44, heroCrest: 140, heroScore: 84, heroStatusFont: 26, heroBadge: 42, heroScoreHalf: 120, heroChanFont: 26, emptyFont: 46 },
  story: { W: 1080, H: 1920, pad: 72, accent: 16, brandDot: 34, brandFont: 40, titleFont: 60, dateFont: 34, tagFont: 28, rowH: 146, rowGap: 18, nameFont: 40, crest: 68, scoreFont: 44, badge: 50, scoreHalf: 96, gap: 20, chanFont: 28, heroH: 470, heroGap: 26, heroCompFont: 32, heroName: 56, heroCrest: 184, heroScore: 112, heroStatusFont: 32, heroBadge: 54, heroScoreHalf: 150, heroChanFont: 32, emptyFont: 58 },
};
...
export function buildImage({ date, n, format, highlight, lang }) {
  ...
  (function () {
    ...
    function pick() {
      ...
      // The selector is the total number of games in the image; the hero counts
      // as one, so the list shows one fewer when a game is featured.
      var num = parseInt(nEl.value, 10) || 5;
      var listCount = hero ? Math.max(0, num - 1) : num;
      return { hero: hero, top: list.slice(0, listCount) };
    }
    ...
    function drawRow(f, y, S) {
      ...
      // Center column, stacked + vertically centred: score/time, the live/FT pill,
      // then the Portuguese TV channel (where to watch).
      var pill = statusPill(f), ch = ptChannelFor(f);
      var sub = Math.round(S.scoreFont * 0.44), g2 = Math.round(S.scoreFont * 0.12) + 2;
      var totalH = S.scoreFont + (pill ? g2 + sub : 0) + (ch ? g2 + S.chanFont : 0);
      var cur = cy - totalH / 2;
      setFont(S.scoreFont, "800"); ctx.fillStyle = C.text; ctx.textAlign = "center"; ctx.textBaseline = "middle";
      ctx.fillText(resultOf(f), cx, cur + S.scoreFont / 2); cur += S.scoreFont;
      if (pill) { cur += g2; setFont(sub, "800"); ctx.fillStyle = pill.c; ctx.fillText(pill.t, cx, cur + sub / 2); cur += sub; }
      if (ch) { cur += g2; setFont(S.chanFont, "700"); ctx.fillStyle = C.accent; ctx.fillText(ellipsize(ch, (S.scoreHalf + S.gap) * 2 - 8), cx, cur + S.chanFont / 2); }
    ...
    function drawHero(f, y, S) {
      ...
      var subY = crestCy + S.heroScore * 0.6;
      if (pill) { setFont(S.heroStatusFont, "800"); ctx.fillStyle = pill.c; ctx.fillText(pill.t, cx, subY); subY += S.heroStatusFont * 1.25; }
      if (ch) { setFont(S.heroChanFont, "700"); ctx.fillStyle = C.accent; ctx.fillText(ellipsize(ch, S.heroScoreHalf * 2), cx, subY); }
    }

    // Proportionally shrink the content block (hero + rows) so it always fits the
    // card height — taller rows (now carrying a channel) or many games scale down
    // instead of overflowing. Header sizes are left untouched.
    function scaleFmt(S, k) {
      var keys = ["rowH", "rowGap", "nameFont", "crest", "scoreFont", "badge", "scoreHalf", "gap", "chanFont",
        "heroH", "heroGap", "heroCompFont", "heroName", "heroCrest", "heroScore", "heroStatusFont", "heroBadge", "heroScoreHalf", "heroChanFont"];
      var o = {}; for (var p in S) o[p] = S[p];
      keys.forEach(function (key) { if (typeof S[key] === "number") o[key] = S[key] * k; });
      return o;
    }

    function blockHeight(hero, top, S) {
      return (hero ? S.heroH + S.heroGap : 0) +
    ...
    function draw(hero, top, S, isToday, d) {
      ...
      var avail = contentBottom - contentTop, SS = S;
      var blockH = blockHeight(hero, top, S);
      if (blockH > avail && blockH > 0) { SS = scaleFmt(S, avail / blockH); blockH = blockHeight(hero, top, SS); }
      var startY = contentTop + Math.max(0, (avail - blockH) / 2);
      if (hero) { drawHero(hero, startY, SS); startY += SS.heroH + SS.heroGap; }
      for (var i = 0; i < top.length; i++) drawRow(top[i], startY + i * (SS.rowH + SS.rowGap), SS);
    }
    ...
    function loadTv(d, token) {
      Promise.all([
        ...
        buildText();
        paint(); // redraw the card now that channels are available
      });
    }
    ...
    function ptChannelFor(f) {
      ...
      if (!pt.length) return "";
      pt.sort(function (x, y) { var px = isPaid(x) ? 1 : 0, py = isPaid(y) ? 1 : 0; return px !== py ? px - py : x.localeCompare(y); });
      return pt[0];
    }
