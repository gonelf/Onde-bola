...
export async function saveAds(list) {
  ...
  try {
    ...
    // Once migrated, loadAds reads only the DB — a KV write here would never be
    // read. Surface the failure; only fall back to KV while still pre-migration.
    if (await isMigrated(ADS_KEY)) throw e;
