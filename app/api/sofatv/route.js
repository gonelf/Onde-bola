...
export async function GET(request) {
  const swr = "public, s-maxage=300, stale-while-revalidate=600";

  if (DISABLED) {
    return Response.json({ tvevent: [] }, { headers: { "Cache-Control": swr } });
  }
  ...
  if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !home || !away) {
    return Response.json({ error: "Pass ?date=YYYY-MM-DD&home=&away=" }, {
  ...
  if (debug) {
    return Response.json({ tvevent, _debug: dbg }, {
      headers: { "X-Cache": "BYPASS", "Cache-Control": swr },
    });
  }
  ...
  return Response.json({ tvevent }, {
    headers: { "X-Cache": "MISS", "Cache-Control": swr },
  });
}
