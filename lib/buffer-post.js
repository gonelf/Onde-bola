...
// input types reachable through `assets` (AssetInput → ImageAssetInput, …), so
// we know exactly how to attach the image. Bounded recursion into *Input types
// only (skips scalars/enums/metadata).
export async function introspectCreatePost() {
  ...
  async function collect(typeName, depth) {
    ...
    for (const f of t.fieldsRaw) {
      ...
      if (n && /Input$/.test(n) && !seen[n] && depth > 0) await collect(n, depth - 1);
    }
  }
  await collect("CreatePostInput", 2);
  if (!order.length) {
