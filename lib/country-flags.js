...
// The UK home nations have subdivision flag emojis (🏴󠁧󠁢󠁳󠁣󠁴󠁿 etc.), but those tag
// sequences fail to render on many platforms (WhatsApp/Windows show a blank black
// flag), so use the reliably-supported UK flag for all of them.
iso("GB", [
  "England", "Inglaterra", "Scotland", "Escócia", "Escocia",
  "Wales", "País de Gales", "Pais de Gales",
  "Great Britain", "United Kingdom", "Reino Unido",
]);
...
// Remaining European (UEFA)
iso("LI", ["Liechtenstein"]);
iso("SM", ["San Marino", "São Marino"]);
iso("GI", ["Gibraltar"]);
iso("KZ", ["Kazakhstan", "Cazaquistão"]);
add(flagFromIso("GB"), ["Northern Ireland", "Irlanda do Norte"]); // no NI flag emoji; use the UK flag

// Remaining CONCACAF / Caribbean
iso("NI", ["Nicaragua", "Nicarágua"]);
iso("DO", ["Dominican Republic", "República Dominicana"]);
iso("SR", ["Suriname", "Suriname"]);
iso("GY", ["Guyana", "Guiana"]);
iso("BZ", ["Belize"]);
iso("BB", ["Barbados"]);
iso("GD", ["Grenada", "Granada"]);
iso("LC", ["St Lucia", "Saint Lucia", "Santa Lúcia"]);
iso("VC", ["St Vincent and the Grenadines", "Saint Vincent and the Grenadines", "St Vincent & Grenadines"]);
iso("KN", ["St Kitts and Nevis", "Saint Kitts and Nevis", "St Kitts & Nevis"]);
iso("AG", ["Antigua and Barbuda", "Antigua & Barbuda"]);
iso("DM", ["Dominica"]);
iso("BM", ["Bermuda", "Bermudas"]);
iso("BS", ["Bahamas"]);
iso("AW", ["Aruba"]);
iso("PR", ["Puerto Rico", "Porto Rico"]);
iso("GP", ["Guadeloupe"]);
iso("MQ", ["Martinique", "Martinica"]);
iso("KY", ["Cayman Islands", "Ilhas Caimão"]);
iso("MS", ["Montserrat"]);
iso("VG", ["British Virgin Islands"]);
iso("VI", ["US Virgin Islands", "U.S. Virgin Islands"]);
iso("TC", ["Turks and Caicos Islands"]);
iso("AI", ["Anguilla"]);

// Remaining CAF
iso("SL", ["Sierra Leone", "Serra Leoa"]);
iso("LR", ["Liberia", "Libéria"]);
iso("NE", ["Niger", "Níger"]);
iso("TD", ["Chad", "Chade"]);
iso("CF", ["Central African Republic", "República Centro-Africana"]);
iso("BI", ["Burundi", "Burúndi"]);
iso("RW", ["Rwanda", "Ruanda"]);
iso("SS", ["South Sudan", "Sudão do Sul"]);
iso("SO", ["Somalia", "Somália"]);
iso("DJ", ["Djibouti", "Djibuti"]);
iso("ER", ["Eritrea", "Eritreia"]);
iso("KM", ["Comoros", "Comores"]);
iso("SC", ["Seychelles", "Seicheles"]);
iso("MU", ["Mauritius", "Maurícia", "Ilhas Maurício"]);
iso("SZ", ["Eswatini", "Swaziland", "Essuatíni"]);
iso("LS", ["Lesotho", "Lesoto"]);
iso("BW", ["Botswana", "Botsuana"]);
iso("GM", ["Gambia", "Gâmbia"]);
iso("ST", ["Sao Tome and Principe", "Sao Tome & Principe", "São Tomé e Príncipe"]);

// Remaining AFC
iso("BD", ["Bangladesh"]);
iso("PK", ["Pakistan", "Paquistão"]);
iso("LK", ["Sri Lanka"]);
iso("NP", ["Nepal"]);
iso("MM", ["Myanmar", "Mianmar"]);
iso("KH", ["Cambodia", "Camboja"]);
iso("LA", ["Laos"]);
iso("SG", ["Singapore", "Singapura"]);
iso("PH", ["Philippines", "Filipinas"]);
iso("HK", ["Hong Kong"]);
iso("TW", ["Chinese Taipei", "Taiwan"]);
iso("MN", ["Mongolia", "Mongólia"]);
iso("KG", ["Kyrgyzstan", "Kyrgyz Republic", "Quirguistão"]);
iso("TJ", ["Tajikistan", "Tajiquistão"]);
iso("TM", ["Turkmenistan", "Turcomenistão"]);
iso("AF", ["Afghanistan", "Afeganistão"]);
iso("YE", ["Yemen", "Iémen", "Iemen"]);
iso("MV", ["Maldives", "Maldivas"]);
iso("BT", ["Bhutan", "Butão"]);
iso("BN", ["Brunei", "Brunei Darussalam"]);
iso("TL", ["Timor-Leste", "East Timor", "Timor Leste"]);
iso("MO", ["Macau", "Macao"]);
iso("GU", ["Guam"]);

// OFC
iso("FJ", ["Fiji"]);
iso("PG", ["Papua New Guinea", "Papua-Nova Guiné"]);
iso("SB", ["Solomon Islands", "Ilhas Salomão"]);
iso("VU", ["Vanuatu"]);
iso("WS", ["Samoa"]);
iso("AS", ["American Samoa", "Samoa Americana"]);
iso("TO", ["Tonga"]);
iso("CK", ["Cook Islands", "Ilhas Cook"]);
iso("PF", ["Tahiti", "French Polynesia"]);
iso("NC", ["New Caledonia", "Nova Caledónia"]);
